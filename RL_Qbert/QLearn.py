...
# Get the list of legal actions, don't want to use do nothing move
...
for z in range(0, 1):
	...
	qbertpos=[]		#used to help determine if in valid frame
	actstolearn = 0

	framecnt=0
	
	rewardlist = []		#used for recording results
	mvcountlist = []	#used for recording results
	growthQtable =[]	#used for recording results
	...
	for episode in range(500):
		...
		print('Game %d Episode %d score %d moves %d size %d' \
			% (z, episode, total_reward, mvcount, qtablesize))

		if((episode+1)%25==0):
			outputfile = "QtabEp"+str(episode)
	...
	qtabfile = "qtabsize"+str(z)
	rewardfile = "rewardgrowth"+str(z)

	qtabSer.to_csv(qtabfile)
	rewSer.to_csv(rewardfile)	
