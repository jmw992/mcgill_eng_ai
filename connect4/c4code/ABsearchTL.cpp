...
Move  ConnectFourGame::absearchTimeLimit(State &SearchState, int depth, Move Alpha, Move Beta) {
	//update searched board list
	searchedboardcount += 1;
	searchedboards.push_back(SearchState.getboard());

	if (depth >= c4maxdepth || SearchState.getterminalstate() ||time(NULL)- starttime > timelimit) {
	...
	if (SearchState.getwhiteturn())  
	{
		...
		for (int i = 0; i < childStates.size(); i++) {
			...
			//V = max(absearch, beta)
			Move TmpMv = absearchTimeLimit(childStates[i], (1 + depth), Alpha, Beta);	//child state already fed opposite turn
